#!/usr/bin/env bun
/**
 * Bundles the `/admin` page: `apps/docs/src/admin/main.ts` → `apps/docs/static/admin/admin.js`.
 *
 ...
 *
 * Usage:
 *   bun run scripts/build/build-admin.ts
...
const ENTRY_POINT = join(REPO_ROOT, 'apps/docs/src/admin/main.ts');
const OUTPUT_DIRECTORY = join(REPO_ROOT, 'apps/docs/static/admin');
const OUTPUT_FILE = join(OUTPUT_DIRECTORY, 'admin.js');
...
async function buildAdmin(): Promise<void> {
  ...
  if (!result.success) {
    ...
    throw new Error(`Could not bundle the /admin page:\n${detail}`);
  }

  // Belt and braces. `success` has been true for a build that wrote nothing when every entrypoint
  // resolved to an empty module, and a silently absent `admin.js` leaves an author looking at
  ...
  console.log(`Bundled the /admin page → ${OUTPUT_FILE} (${String(Math.round(bytes / 1024))} kB)`);
}
